...
#    loss_conf  = tf.reduce_sum(tf.square(true_box_conf-pred_box_conf) * conf_mask)  / (nb_conf_box  + 1e-6) / 2. #problematic? lambda_1_loss/mul_20?
    loss_class = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=true_box_class, logits=pred_box_class)
    loss_class = tf.reduce_sum(loss_class * class_mask) / (nb_class_box + 1e-6)

    #loss = loss_xy + loss_z + loss_wl + loss_h + loss_yaw + loss_conf + loss_class
    loss = loss_xy + loss_z + loss_wl + loss_h + loss_yaw + loss_class
